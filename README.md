...
**What is it?**
Splunk is a Security Information Events Manager (SIEM) used to collect and index logs from multiple devices (firewalls, Endpoints, Routers, etc) to enable search, correlation, and monitoring all on a single interface. 
...
- Splunk Universal Forwarder were set up for the **Windows 10 Endpoint**
...
<p align="center"><b>Figure 2: Splunk Receiving Port 9997 setup for Universal Forwarders to send logs to</b></p>
...
**Use Case:**
A Windows 10 Endpoint was configured within the LAN to able to forward logs (including failed authentication attempts) to our spunk server. This machine was also used to access the N8N UI for configuring workflow nodes and setup the slack channel for recieving alerts generated by OpenAI.
...
**Use Case:**
In this project N8N is used (using Ubuntu Live Server) to automate the process of repetitive tasks with analysing alerts created by splunk and perform enrichment with IP Addresses and Hashes using IPDBAbuse and VirusTotal. 

**Configuration**
- Installed Docker & Docker-compose
- Created docker-compose.yml to install the latest version if N8N(*Figure 6*)
- Deployed N8N
- Configured the Webhook to receive alerts from splunk (*Figure 7*)
...
<p align="center"><b>Figure 7: N8N Webhook successfully recieving Splunk Alert.</b></p>

<p align="center">
  <img width="664" height="389" alt="image" src="https://github.com/user-attachments/assets/89fc67eb-d5ca-44df-b607-a5041f5bc332" />
</p>
<p align="center"><b>Figure 8: N8N Automation Workflow integrating Webhook Ingestion, AI Analysis, IOC/Hash Enrichment, and Slack Notification</b></p>
...
This section covers the workflow, more specifically the OpenAI's Message a Model Node to automatically analyse incoming Splunk Alert. The goal is to replicate the actions of a tier 1 SOC analyst by summarising the alert, enriching indicators, mapping MITRE ATT&CK techniques, and generating recommended next steps. 

**Prompt Engineering

to ensure consistent and structured output, multiple prompts were configured
...
- Recommended Actions
- It also specifies which tools the model should use (AbuseIPDB for IPs, and VirusTotal for file Hashes)

2. **User Prompt**

This prompt controls Splunk's data being passed into the LLM (*Figure 9*)
...
- VirusTotal Hash Lookup Tool - This is used to analyse any suspicious file hashes found in the alert
  - These tools are directly referenced in the Assistant Prompt so it knows when to use it.
...
- A dedicated channel (called **splunk-alerts**) was setup to receive automated notifications
- A Slack App was created to generate the OAuth Bot token used by N8N to authenticate and send messages into the Slack Channel.
...
For testing purposes, following IP Address (173.244.55.102) was hardcoded into the user prompt. This allows us to validate that the OpenAI node can correctly identify, enrich, and analyse malicious IOCs even though our virtual machine is not exposed to the internet (*Figure 11*). 
...
- From the slack message we can see that IP Enrichment was successful providing us with a confidence score and how many times it has been reported.
- A severity assessment was generated automatically based on the IOC and alert context
- Recommended actions were produced, giving clear investigative and containment steps
...
For testing purposes, a file hash was hardcoded into our user prompt. Mainly because with failed logons we will not get File Hashes (*Figure 13*). This File Hash was obtained from [the DFIR Report  ](https://thedfirreport.com/)

Upon executing our entire automation workflow a slack message was sent to the soc-alerts channel by our bot (*Figure 14*)
- From the Slack message we can see that the File Hash enrichment was sucessful, providing us detail from VirusTotal into the capabilities into the file.
...
This project successfully demonstrated how a modern SOC can leverage automation, AI, and enrichment tooling to accelerate incident triage, and improve response efficiency. By intergrating Splunk, N8N, OpenAI, AbuseIPDB, and VirusTotal, we built an end-to-end pipeline capable of transforming raw security events into fully enriched, structured, and actionable reports. 

The workflow processed both Network-Based IOCs and File-Based IOCs automatically generating severity assessments, MITRE ATT&CK mappings and recommended response actions. This ultimately reduces analyst workload, improves consistency in triage, and enable faster visibility inro potential threats.
